...
public class SwipeButton extends RelativeLayout {
  ...
  private boolean hasFinishAnimationBack = false;
  private boolean hasFiredActive = false;
  ...
  public void setOuterBackgroundDrawable(Drawable drawable) {
    if (drawable != null) background.setBackground(drawable);
    else
      background.setBackground(
          ContextCompat.getDrawable(context, R.drawable.swipe_rounded_background));
  }

  public void setOuterBackgroundTint(int color) {
    if (color != -1) background.setBackgroundTintList(ColorStateList.valueOf(color));
  }
  ...
  public void setButtonBackgroundDrawable(Drawable drawable) {
    if (drawable != null) swipeBtn.setBackground(drawable);
    else
      swipeBtn.setBackground(
          ContextCompat.getDrawable(context, R.drawable.swipe_button_background));
  }

  public void setButtonBackgroundTint(int color) {
    if (color != -1) swipeBtn.setBackgroundTintList(ColorStateList.valueOf(color));
  }
    ...
    if (attrs != null && defStyleAttr == -1 && defStyleRes == -1) {
      TypedArray typedArray =
          context.obtainStyledAttributes(
              attrs, R.styleable.SwipeButton, defStyleAttr, R.style.default_swipe_button_style);
      // ----- HANDLE THE OUTER BACKGROUND -----//
      setOuterBackgroundDrawable(
          typedArray.getDrawable(R.styleable.SwipeButton_outer_background_drawable));
      setOuterBackgroundTint(
          typedArray.getColor(R.styleable.SwipeButton_outer_background_tint, -1));
      ...
      centerText.setTextColor(
          typedArray.getColor(R.styleable.SwipeButton_inner_text_color, Color.WHITE));

      float innerTextPadding =
          typedArray.getDimension(R.styleable.SwipeButton_inner_text_padding, -1);
      float innerTextLeftPadding =
          typedArray.getDimension(R.styleable.SwipeButton_inner_text_left_padding, 0);
      float innerTextTopPadding =
          typedArray.getDimension(R.styleable.SwipeButton_inner_text_top_padding, 0);
      float innerTextRightPadding =
          typedArray.getDimension(R.styleable.SwipeButton_inner_text_right_padding, 0);
      float innerTextBottomPadding =
          typedArray.getDimension(R.styleable.SwipeButton_inner_text_bottom_padding, 0);
      ...
      collapsedWidth =
          (int)
              typedArray.getDimension(
                  R.styleable.SwipeButton_button_width, ViewGroup.LayoutParams.WRAP_CONTENT);
      collapsedHeight =
          (int)
              typedArray.getDimension(
                  R.styleable.SwipeButton_button_height, ViewGroup.LayoutParams.WRAP_CONTENT);

      setButtonBackgroundDrawable(
          typedArray.getDrawable(R.styleable.SwipeButton_button_background_drawable));
      setButtonBackgroundTint(
          typedArray.getColor(R.styleable.SwipeButton_button_background_tint, -1));
      setButtonBackgroundImage(
          typedArray.getDrawable(R.styleable.SwipeButton_button_background_src));
      ...
      setButtonPadding(
          (int) typedArray.getDimension(R.styleable.SwipeButton_button_background_padding, 0));
      swipeBtn.setElevation(1);

      // ----- HANDLE THE TRAIL -----//
      trailEnabled = typedArray.getBoolean(R.styleable.SwipeButton_trail_enabled, false);
      int trailBackgroundTint =
          typedArray.getColor(
              R.styleable.SwipeButton_trail_background_tint, getResources().getColor(R.color.gray));
      Drawable trailBackgroundDrawable =
          typedArray.getDrawable(R.styleable.SwipeButton_outer_background_drawable);
      ...
      if (trailEnabled) {
        ...
        if (trailBackgroundDrawable != null) trail.setBackground(trailBackgroundDrawable);
        else
          trail.setBackground(
              ContextCompat.getDrawable(context, R.drawable.swipe_rounded_background));
      ...
      hasFinishAnimation =
          typedArray.getBoolean(R.styleable.SwipeButton_has_finish_animation, true);
      hasFinishAnimationBack =
          typedArray.getBoolean(R.styleable.SwipeButton_has_finish_animation_back, false);
      typedArray.recycle();
    }
      ...
      public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
          ...
          case MotionEvent.ACTION_MOVE:
            float newX = event.getX() - swipeBtn.getWidth() / 2;
            float maxX = getWidth() - swipeBtn.getWidth();

            // Batasi swipeBtn agar tidak keluar dari parent
            if (newX < 0) newX = 0;
          ...
          case MotionEvent.ACTION_UP:
            boolean isSwipedFull =
                swipeBtn.getX() + swipeBtn.getWidth() > background.getWidth() * 0.9;
            if (isSwipedFull) {
              if (!hasFiredActive) {
                moveButtonToEnd(); // animasi ke kanan
              }
            } else {
              moveButtonBack(); // animasi ke kiri + reset status
            }
            return true;
        }
        return false;
  ...
  private void activateButton() {
    final ValueAnimator positionAnimator =
        ValueAnimator.ofFloat(swipeBtn.getX(), background.getX());
    ...
    final ValueAnimator widthAnimator = ValueAnimator.ofInt(collapsedWidth, background.getWidth());
  ...
  private void deactivateButton() {
    final ValueAnimator widthAnimator = ValueAnimator.ofInt(swipeBtn.getWidth(), collapsedWidth);
  ...
  private void expandTrail() {
    ...
    trailLayoutParams.width = (int) (swipeBtn.getX() + swipeBtn.getWidth()); // gunakan width tombol
  ...
  private void moveButtonBack() {
    ...
    AnimatorSet animatorSet = new AnimatorSet();
    animatorSet.playTogether(ObjectAnimator.ofFloat(centerText, "alpha", 1), positionAnimator);
    animatorSet.addListener(
        new AnimatorListenerAdapter() {
          @Override
          public void onAnimationEnd(Animator animation) {
  ...
  private void moveButtonToEnd() {
    final float endX = getWidth() - swipeBtn.getWidth();
    final ValueAnimator positionAnimator = ValueAnimator.ofFloat(swipeBtn.getX(), endX);
    ...
    animatorSet.addListener(
        new AnimatorListenerAdapter() {
          ...
          public void onAnimationEnd(Animator animation) {
            hasFiredActive = true;
            isActive = true;
            if (onActiveListener != null) onActiveListener.onActive();
          }
        });
  ...
  public void startHintAnimation() {
    post(
        () -> {
          final float originalX = swipeBtn.getX();
          final float distance = getWidth() * 0.15f; // jarak animasi ke kanan
          final int repeatCount = 1;
          final long duration = 500;

          AnimatorSet fullSet = new AnimatorSet();
          AnimatorSet[] animations = new AnimatorSet[repeatCount];

          for (int i = 0; i < repeatCount; i++) {
            ValueAnimator goRight = ValueAnimator.ofFloat(originalX, originalX + distance);
            goRight.setDuration(duration);
            goRight.addUpdateListener(
                anim -> {
            ...
            ValueAnimator goLeft = ValueAnimator.ofFloat(originalX + distance, originalX);
            goLeft.setDuration(duration);
            goLeft.addUpdateListener(
                anim -> {
                  float value = (float) anim.getAnimatedValue();
                  swipeBtn.setX(value);
                  expandTrail();
            ...
            AnimatorSet oneCycle = new AnimatorSet();
            oneCycle.playSequentially(goRight, goLeft);
            animations[i] = oneCycle;
          }

          fullSet.playSequentially(animations);
          fullSet.start();
        });
  }
}
